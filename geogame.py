...
class GeoGame:
    ...
    def get_daily_country(self):
        # 1. Get the current date in strict GMT (UTC)
        current_date = datetime.datetime.now(datetime.timezone.utc).date()
        
        # 2. Create a custom random number generator seeded with today's date string (e.g., "2026-04-18")
        # Because the seed is the same all day, it will always output the exact same number!
        daily_rng = random.Random(str(current_date))
        
        # 3. Pick the country using the daily seeded number
        random_number = daily_rng.randint(0, len(self.df_countries) - 1)
    ...
    def __init__(self):
        ...
        self.target = self.get_daily_country()
